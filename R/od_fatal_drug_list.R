...
od_fatal_drug_list <- function(data, underly_col, mult_col) {
    data %>% mutate(cdc_any_drugs = od_create_diag(., expr = "X4[0-4]|X6[0-4]|X85|Y1[0-4]", 
        colvec = underly_col)) %>% mutate(
        cdc_any_opioid = od_create_cond_diag(., expr = "T40[0-46]", colvec = mult_col, cond.var = cdc_any_drugs), 
        non_heroin_opioid = od_create_cond_diag(., expr = "T40[02-46]", colvec = mult_col, cond.var = cdc_any_drugs),
        ...
        rx_opioid_not_fentanyl = od_create_cond_diag(., expr = "T40[2-3]", colvec = mult_col, cond.var = cdc_any_drugs), 
