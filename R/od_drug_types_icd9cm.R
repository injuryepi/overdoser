od_drug_types_icd9cm <- function(data, diag_ecode_col){
	...
	data %>%
		mutate(any_drug_icd9cm =
					 	create_diag(., expr = cdc_drugs_icd9cm_regex_,
					 							colvec = diag_ecode_col),
					 any_opioid_icd9cm =
					 	create_diag(., expr = cdc_opioid_icd9cm_regex_,
					 							colvec = diag_ecode_col),
					 non_heroin_icd9cm =
					 	create_diag(., expr = cdc_non_heroin_icd9cm_regex_,
					 							colvec = diag_ecode_col),
					 heroin_icd9cm =
					 	create_diag(., expr = cdc_heroin_icd9cm_regex_,
					 							colvec = diag_ecode_col)
		)
}
