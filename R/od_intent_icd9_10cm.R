...
od_intent_icd9_10cm <- function(data, diag_ecode_col) {
    ...
    data %>% mutate(unintentional_drugs_icd9_10cm = od_create_diag(., expr = unintentional_drugs_icd9_10cm_regex_, 
        colvec = diag_ecode_col), self_harm_drugs_icd9_10cm = od_create_diag(., 
        expr = self_harm_drugs_icd9_10cm_regex_, colvec = diag_ecode_col), 
        assault_drugs_icd9_10cm = od_create_diag(., expr = assault_drugs_icd9_10cm_regex_, 
            colvec = diag_ecode_col), undetermined_drugs_icd9_10cm = od_create_diag(., 
            expr = undetermined_drugs_icd9_10cm_regex_, colvec = diag_ecode_col)) %>% 
        mutate(intent_drugs = ifelse(unintentional_drugs_icd9_10cm == 1, 1, 
            ifelse(self_harm_drugs_icd9_10cm == 1, 2, ifelse(assault_drugs_icd9_10cm == 
                1, 3, ifelse(undetermined_drugs_icd9_10cm == 1, 4, NA))))) %>% 
